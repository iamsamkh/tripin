...
class CommentsBloc extends ChangeNotifier {
  ...
  Future saveNewComment(
      String collectionName, String id, String newComment) async {
    ...
    await _getDate()
        .then((value) => FirebaseFirestore.instance
                .collection('$collectionName/$id/comments')
                .doc('$_uid$timestamp1')
                .set({
              'name': _name,
              'comment': newComment,
              'date': date,
              'image url': _imageUrl,
              'reviewTime': timestamp1,
              'uid': _uid
            }))
        .then((value) {
      if (collectionName == 'placesN') {
        commentInrement(collectionName, id);
      }
    });
  ...
  Future commentInrement(String collectionName, String id) async {
    final DocumentReference ref =
        FirebaseFirestore.instance.collection(collectionName).doc(id);
    DocumentSnapshot snap = await ref.get();
    int _commentsAmount = snap['experienceCount'];
    await ref.update({'experienceCount': _commentsAmount + 1});
  }
